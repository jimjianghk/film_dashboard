...
A floating liquid-glass pill, fixed top-center of the viewport, holds one short tab per chart section. It slides in once the charts reach the upper third of the viewport, i.e. once the title and stats row have scrolled past, and slides away again above that point. A scroll-spy lights the tab for the chart currently in view, and clicking a tab scrolls to its chart and records the section in the URL fragment, so refreshing or sharing the link returns to the same chart.
...
On the current year's dashboard, a "Show forecast" switch in the chart header (off by default, styled like the dedup toggles) extends the chart through December 31 and sketches how the year might finish. Each past year implies a year-end total, the current count scaled by how far through its own total that year was on the same date; the forecast blends the average of those implied totals with the average of the past totals themselves, each weighted by its steadiness, spreads the remainder across the coming months in past years' average monthly rhythm so the curve keeps the season's shape, and draws a salmon confidence area spanning a 95% prediction interval that tightens as past years accumulate and the year fills in. The pool of past years reaches back to 2017, with 2020 and 2021 sitting out as covid-distorted; the pre-2022 years survive only as daily counts, so they inform the forecast without appearing as archive views. The forecast span takes *The Economist's* house treatment, a grey band beneath a small "Forecast" label with the line running solid across it, and the y-axis rescales to make room; toggling animates the change, instantly for users who prefer reduced motion. The switch stays hidden until the data day reaches March 1, before which the blend would lean almost entirely on past years rather than the season so far, and on archive views, where the year is already complete.

A cursor trace reveals the cumulative count for any day from January 1 through the data day. A thin vertical line and a small accent-colored dot snap to the nearest day, and a single-line thin-border tooltip shows the date and the screening count. On desktop the trace follows the mouse and disappears when the cursor leaves the chart. On touch, a press-and-drag traces; releasing leaves the tooltip in place until the user taps elsewhere. Hovering or tapping past the data-day marker is treated as outside the chart and hides the trace, but if a continuous touch drag crosses past the marker the tooltip clamps to the data day and persists, including after the finger lifts. With the forecast on, the trace keeps working across the forecast span, where the tooltip shows the forecast screening count instead.
...
- Free-text search on title that works with CJK input methods and finds a bilingually logged film by either title.
...
- The Format and Price column headers carry footnote markers tying to the page footnotes (DCP includes other digital; per-row prices exclude membership fees), taking whichever symbols the page-order sequence assigns.
...
A row of small notes between the log table and the footer carries the symbol callouts referenced from the hero "Total spending" stat, the forecast switch on the cumulative chart, the decade chart's Various bar, the Format column (and the format chart's DCP row), and the Price column header. Symbols run in a fixed sequence (asterisk, dagger, double dagger, chapter sign, doubled asterisk) assigned in page order to whichever notes are present: the forecast note, stating the counting basis of the estimates and the band's 95% meaning, appears only while its switch does, and the Various note only when multi-decade shorts programs exist, with the remaining symbols shifting to close the gaps and every marker following suit. When the year includes foreign-currency tickets, the asterisk note extends to explain that non-USD prices are converted using market exchange rates. The notes sit inline, evenly spaced.
...
A Chinese-language film is logged with its original and English titles in one cell, separated by " || " (e.g. "流浪地球 2 || The Wandering Earth 2"). The exporter fails the sync on a half-empty pair rather than export a broken title. The combined string carries through the JSON unchanged, and the log table is the only place the English half is displayed; the calendar tooltip and every other view show the original title alone.

A screening paid in a foreign currency is logged in the same price cell as an ISO currency code plus the local amount (e.g. "CNY50", "EUR14.50"). The exporter converts it to USD at the screening date's ECB reference rate via the Frankfurter API, with weekend and holiday screenings using the most recent business day's rate. The exported price stays in USD, so totals, comparisons, and sorting share one currency, while the original amount is kept alongside for the log table's local-price display. Fetched rates are cached in the committed `data/fx-rates.json`, so past conversions never drift across reruns and the sync survives API outages; a rate can be added there by hand if a currency falls outside ECB coverage. A screening synced before its date's rate has published converts provisionally at the latest available rate, and a later sync locks in the date's own rate. Any other text in a price cell fails the sync loudly rather than exporting a broken price.
