...
During June, a slim five-stripe pride accent in a softened rainbow palette extends from the right edge of the masthead without shrinking the logo. The accent is gated on the browser's real current month in New York time, independent of the dashboard year or its data. On page load the stripes drop into place one at a time, left to right, using the same easing as the page's entrance animations; the cascade is disabled under `prefers-reduced-motion`.
