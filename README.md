...
Full-width line chart tracking the running total of screenings over the year. The line stays flat between film days and slopes upward only across the day a screening was logged. The line renders clipped to the plot area, so its rounded stroke ends never spill past the axes at January 1 or, once the chart runs through the year end, at December 31. The x-axis starts at January 1 and extends a little past the latest data so the line isn't crammed against the right edge; after the dashboard year has ended it runs through December 31. Month labels sit centered between consecutive month-start ticks, and the bottom axis line and ticks match the runtime histogram. On narrow mobile screens, when the axis spans most of the year, month labels collapse to single letters (J, F, M, ...) to keep them legible. The y-axis rounds up to a clean tick value, with right-aligned labels and faint horizontal gridlines. A dashed vertical marker indicates the most recent "data day": today in New York if a screening has been logged today, otherwise yesterday. When the marker falls on yesterday, the line terminates flat at that point regardless of whether yesterday had any screenings. If the chart extends through December 31, the marker is hidden.

On the current year's dashboard, a "Show forecast" switch in the chart header (off by default, styled like the dedup toggles) extends the chart through December 31 and sketches how the year might finish. The forecast total scales the current count by how far through their final totals past years were at this same date, normalizing Feb. 29 like the stats row, and the remainder is spread across the coming months following past years' average monthly rhythm, so the forecast curve keeps the season's shape instead of climbing in a straight line. The forecast span takes *The Economist's* house treatment: a grey band shades the rest of the year beneath a small "Forecast" label, which slips to just left of the band once the remaining span is too narrow to hold it, the band's left edge taking over the data-day marker's job while gridlines pass through the band unchanged; the line runs solid across the band, and a salmon confidence area, tinting the gridlines it crosses, spans a 95% prediction interval for the year-end total, computed with Student's t from the spread of the totals that past years' paces imply, so it tightens as archive years accumulate and as the year fills in. Band, salmon, and label colours are the exact sRGB values extracted from the print original, so the treatment composites on the site's paper cream precisely as it does on the page. The x-axis stretches to the full year and the y-axis rescales to fit the confidence area's ceiling, leaving the plot's footprint unchanged. Toggling animates between the states with the bar charts' easing: the axes rescale gradually while the forecast layer, the incoming axis ticks, and the extra month labels fade in or out alongside, with the forecast cropped to the plot area throughout, and the transition collapses to an instant switch for users who prefer reduced motion. The switch stays hidden until the data day reaches April 1, before which the interval comes out wider than simply predicting a typical past year, and on archive views, where the year is already complete.
