...
A row of small notes between the log table and the footer carries the symbol callouts referenced from the hero "Total spending" stat, the forecast switch on the cumulative chart, the decade chart's Various bar, the Format column (and the format chart's DCP row), and the Price column header. Symbols run in a fixed sequence (asterisk, dagger, double dagger, chapter sign, then those same four doubled) assigned in page order to whichever notes are present: the forecast note, stating the counting basis of the estimates and the band's 95% meaning, appears only while its switch does, and the Various note only when multi-decade shorts programs exist, with the remaining symbols shifting to close the gaps and every marker following suit. When the year includes foreign-currency tickets, the asterisk note extends to explain that non-USD prices are converted using market exchange rates. The notes sit inline, evenly spaced.
