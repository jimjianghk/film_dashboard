...
- Chromium browsers add true edge refraction on top: each glass element gets a runtime-generated SVG displacement map (a signed-distance-field lens that keeps the center optically flat and bends the backdrop outward across a rim band), applied via `backdrop-filter: url()` with three slightly diverging passes recombined per color channel for a prismatic fringe, plus `corner-shape: squircle` where supported; this tier is gated by engine detection because Safari and Firefox parse but silently ignore SVG filters in `backdrop-filter`
